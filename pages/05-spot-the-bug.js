...
export const SpotTheBug = () => {
  ...
  const handleFiltersPlayersByClub = (clubName) => {
    ...
    if (clubName === 'No club') {
      return setFilteredPlayers(footballPlayers.filter((player) => !player.club));
    }
    setFilteredPlayers(footballPlayers.filter((player) => player.club === clubName));
  };


  useEffect(() => {
    // Set clubs with unique values from footballPlayers
    setClubs([...new Set(footballPlayers
      .map((player) => player.club))]
      .map((club) => club || 'No club')
      );
  ...
  return (
    <>
      <p className={styles.challengeDescription}>
        Complete the challenge in the <code>pages/05-spot-the-bug.js</code> file.<br/>
      </p>
